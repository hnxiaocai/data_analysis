...
class Rent(object):
    ...
    def __init__(self):
        ...
        # db = pymysql.connect(host='localhost', user='root', passwd='usbw', port=3306)
        # cur = db.cursor()
        # cur.execute("CREATE DATABASE IF NOT EXISTS lianjia DEFAULT CHARACTER SET utf8")
        # cur.execute('use lianjia')
        # cur.execute("create table if not exists zufang ( bedroom_num int, hall_num int, bathroom_num int, rent_area int, house_title varchar(30), resblock_name varchar(30), bizcircle_name varchar(30), layout varchar(30), rent_price_listing int, house_tag varchar(30), frame_orientation varchar(30), m_url varchar(255) primary key, rent_price_unit varchar(10), latitude double(14, 12), longitude double(14, 11), distance int)")
...
###################################################SQL在这里改
                ...
                sql = "insert ignore into zufang(%s) values(%s)"
                ...
                cur.execute(res_sql, item)
                db.commit()

                # self.db['zufang'].update_one({'m_url': item['m_url']}, {'$set': item}, upsert=True)#'$set': item，用来指定一个键并更新键值，若键不存在并创建
